{{-- Featured Image Component for Pages --}}
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
    ...
    @if(isset($page) && $page->featured_image)
        <div class="mb-4">
            <img src="{{ Storage::url($page->featured_image) }}" alt="Current image" class="w-full h-48 object-cover rounded-lg">
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">{{ __('admin.pages.current_image') }}</p>
        </div>
    @endif
    ...
    <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center hover:border-gray-400 dark:hover:border-gray-500 transition-colors">
        ...
        <label for="featured_image" class="cursor-pointer">
            ...
            <div class="mt-2">
                <p class="font-medium">{{ __('admin.pages.upload_image') }}</p>
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    <span class="font-medium text-blue-600 hover:text-blue-500">{{ __('admin.pages.upload_image') }}</span>
                    {{ __('admin.pages.drag_and_drop') }}
                </p>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    {{ __('admin.pages.file_types') }}
                </p>
            </div>
        </label>
    </div>
